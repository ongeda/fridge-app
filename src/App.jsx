...
function PCard({ p, alertDays, onDel, onQty }) {
  ...
  if (consumed) {
    ...
    bs = { color: '#999', background: '#f0f0f0', border: '0.5px solid rgba(0,0,0,0.1)' }; bl = '소진됨'
  } else if (expired) {
    cs = { background: '#FCEBEB', border: '0.5px solid #F7C1C1' }
    bs = { color: '#A32D2D', background: '#FCEBEB', border: '0.5px solid #F7C1C1' }; bl = `만료 ${Math.abs(dd)}일 경과`
  } else if (urgent) {
    cs = { background: '#FFFDF7', border: '0.5px solid #FAC775' }
    bs = { color: '#854F0B', background: '#FAEEDA', border: '0.5px solid #FAC775' }; bl = dd === 0 ? 'D-day' : `D-${dd}`
  } else {
    cs = { background: '#fff', border: '0.5px solid rgba(0,0,0,0.1)' }
    bs = { color: '#666', background: '#f5f5f5', border: '0.5px solid rgba(0,0,0,0.1)' }; bl = `D-${dd}`
  }
  return (
